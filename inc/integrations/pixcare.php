			...
			<div class="stepper">
				<div class="stepper__content">
					...
					<div class="section section--informative">
						...
						<h2 class="section__title u-text-center"><?php esc_html_e( 'One more lap to go..', 'boilerplate' ); ?></h2>
						<div class="section__content">
							<?php
							echo sprintf(
								'%s <strong>%s</strong> %s',
								esc_html__( 'Start your digital journey with our', 'boilerplate' ),
								esc_html__( 'Pixelgrade Care&#174;', 'boilerplate' ),
								esc_html__( 'on board. It\'s the core of your experience with our WordPress theme, and we bet you are going to love it from day one. You\'re in great hands, no doubt about that!', 'boilerplate' )
							); ?>
						</div>
						<div class="plugins">
							<div class="plugin  box  box--neutral  box--plugin-missing  box--plugin-invalidated">
								<div class="box__body">
									<h5 class="box__title"><?php esc_html_e( 'Pixelgrade Care&#174;', 'boilerplate' ) ?></h5>
									<p class="box__text"><?php esc_html_e( 'Installing ...', 'boilerplate' ) ?></p>
								</div>
							</div>
						</div>
					</div>
					...
					<a class="btn  btn--text  btn--return-to-dashboard"
					   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'boilerplate' ) ?></a>
				</div>
			</div>
		</div>
	</div>
        ...
        (function ($) {
            $(document).ready(function () {
                ...
                wp.ajax.send({type: 'GET'}).always(function (res) {
                    var success = res.indexOf('<p><?php esc_html_e( 'Plugin installed successfully.', 'boilerplate' ); ?></p>');
                    var already_exists = res.indexOf('<p><?php esc_html_e( 'Plugin destination folder already exists.', 'boilerplate' ); ?></p>');
                    ...
                    if (success !== -1) {
                        ...
                        $status.text('<?php esc_html_e( 'Bummer! Something went wrong. I\'m sorry but you will need to install the plugin manually. Redirecting you to the appropriate page...', 'boilerplate' ); ?>');
...
function boilerplate_pixcare_install_page_title( $title ) {
	...
	return esc_html__( 'Pixelgrade Care &rsaquo; Installer', 'boilerplate' );
}
