...
class Pixelgrade_Hero {
	...
	public $_version  = '1.0.15';
	public $_assets_version = '1.0.4';
		...
		$hero_metaboxes = array(
			...
			'hero_area_content__page'     => array(
				...
				'title'      => '&#x1f535; ' . esc_html__( 'Hero Area &#187; Content', 'components' )
				                ...
				                . esc_html__( 'Hero Area &#187; Content', 'components' )
				                . '</title><p>'
				                . wp_kses( __( 'Use this section to add a <strong>Title</strong> or a summary for this page. Get creative and add different elements like buttons, logos or other headings.', 'components' ), wp_kses_allowed_html() )
				                . '</p><p>'
				                . wp_kses( __( 'You can insert a title using a <strong>Heading 1</strong> element, either on the Hero Area or using a <b>Text Block</b> within the above content area.', 'components' ), wp_kses_allowed_html() )
				                . '</p><p>'
				                . wp_kses( __( '* Note that the <strong>Page Title</strong> written above will <u>not</u> be included automatically on the page, so you have complete freedom in choosing where you place or how it looks.', 'components' ), wp_kses_allowed_html() )
				                . "</p><p><a href='#'>"
				                . esc_html__( 'Learn more about Managing the Hero Area', 'components' )
				                . '</a></p>"></span>',
				'pages'      => array( 'page' ), // Post type
				'context'    => 'normal',
				'priority'   => 'high',
				...
				'fields'     => array(
					...
					array(
						...
						'desc'    => wp_kses( __( '<p>Considering the background image focal point, you can align the content to make them both more visible.</p>
							...
							</ul>', 'components' ), wp_kses_allowed_html() ),
					...
					array(
						...
						'value'   => wp_kses( __( 'Add more than one image to the <strong>Hero Area &#187; Background</strong> to enable this section. ', 'components' ), wp_kses_allowed_html() ),
						'type'    => 'title',
					),
					array(
			...
			'hero_area_map__page' => array(
				...
				'fields'     => array(
					...
					array(
						...
						'desc' => wp_kses( __( 'Paste here the <strong>Share Link</strong>URL you have copied from <a href="https://www.google.com/maps" target="_blank">Google Maps</a>. Do not use the embed code or a short URL.', 'components' ), wp_kses_allowed_html() ),
						'id'   => '_hero_map_url',
						'type' => 'textarea_small',
						'std'  => '',
					),
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'rellax', pixelgrade_get_theme_file_uri( 'components/hero/js/jquery.rellax.js' ), array( 'jquery' ), $this->_assets_version, true );
	...
	public function admin_init() {
		...
		wp_register_style( 'pixelgrade_hero-admin-style', pixelgrade_get_theme_file_uri( 'components/hero/css/admin.css' ), array(), $this->_assets_version );
		wp_register_script( 'pixelgrade_hero-admin-scripts', pixelgrade_get_theme_file_uri( 'components/hero/js/admin.js' ), array(), $this->_assets_version );
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( 'edit.php' != $hook ) {
			...
			add_editor_style( array( pixelgrade_get_theme_file_uri( 'components/hero/css/editor-style.css' ) ) );
		}
	}
	...
	public function _add_video_url_field_to_attachments( $form_fields, $post ) {
		...
		if ( ! isset( $form_fields['video_url'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_video_url' ) {
			$form_fields['video_url'] = array(
				...
				'helps' => '<p class="desc">' . wp_kses( __( 'Attach a video to this image <span class="small">(YouTube or Vimeo)</span>.', 'components' ), wp_kses_allowed_html() ) . '</p>',
			);
		}
		...
		if ( ! isset( $form_fields['custom_image_url'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'custom_image_url' ) {
			$form_fields['custom_image_url'] = array(
				...
				'helps' => '<p class="desc">' . esc_html__( 'Link this image to a custom url.', 'components' ) . '</p>',
			);
		}
		...
		if ( ! isset( $form_fields['external_url'] ) && ! empty( $link_media_to_value ) && $link_media_to_value == 'external' ) {
			$form_fields['external_url'] = array(
				...
				'helps' => '<p class="desc">' . esc_html__( 'Set this image to link to an external website.', 'components' ) . '</p>',
			);
		}
